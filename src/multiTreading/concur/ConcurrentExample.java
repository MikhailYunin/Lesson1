...
//    пул потоков
//    альтернативы wait() notify() и synchronized()
    public static void main(String[] args) {

        LinkedBlockingQueue queue;
        LinkedBlockingDeque deque;

        LinkedTransferQueue transferQueue; // блокирует добавление в процессе для взятия

        PriorityBlockingQueue priorityBlockingQueue;

        // нельзя изменить размер
        ArrayBlockingQueue arrayBlockingQueue = new ArrayBlockingQueue(4,true); // флаг для установления порядка
...
//        CopyOnWriteArrayList вмеcто ArrayList
        ...
        map1.putIfAbsent("qwe", 4); // положить, если пусто
        map1.remove("qwe", 4); // удалить, если такая пара есть
        map1.replace("qwe", 4, 86);
        map1.replace("qwe", 86);
